...
# Reverse filenames.
#
# NOTE: This script requires Python3.8 to be installed on your system.
...
class Ek(object):
    ...
    def fix_list_encoding(x):
        ...
        if type(x) not in (list, tuple):
            return x
        return [i for i in map(Ek.fix_out_encoding, x) if None is not i]
    ...
    for arg in exc.args:
        ...
        if None is not arg:
            if isinstance(arg, (str, text_type)):
                ...
                try:
                    fixed_arg = 'error ' + Ek.fix_string_encoding(str(arg))
